...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        ############################################################################
        pass
        forward_value=np.dot(prev_h,self.params[self.wh_name])+np.dot(x,self.params[self.wx_name])+self.params[self.b_name]
        
        next_h=np.tanh(forward_value)
        meta=[x,prev_h,next_h,forward_value]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        return next_h, meta
    ...
    def step_backward(self, dnext_h, meta):
        ...
        #############################################################################
        pass
        dff=(1-np.tanh(meta[3])**2)*dnext_h
        dnext_h=dff
        dx=np.dot(dnext_h,self.params[self.wx_name].T)
        dprev_h=np.dot(dnext_h,self.params[self.wh_name].T)
        dWx=np.dot(meta[0].T,dnext_h)
        dWh=np.dot(meta[1].T,dnext_h)
    ...
    def forward(self, x, h0):
        ...
        for i in range(x.shape[0]):
            h[i],meta=self.step_forward(x[i],h0)
    ...
    def backward(self, dh):
        ...
        input_backward=dh[-1]
        for i in range(dh0.shape[0],0,-1):
            dxi, dprev_h, dWx, dWh, db=self.step_backward(input_backward,self.meta[i-1])
            self.grads[self.wx_name]+=dWx
            self.grads[self.wh_name]+=dWh
            self.grads[self.b_name] +=db
            dh0=dprev_h
            input_backward=dh[i-2]+dprev_h
            dx[i-1]+=dxi
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        #############################################################################
        pass
        activation=np.dot(prev_h,self.params[self.wh_name])+np.dot(x,self.params[self.wx_name])+self.params[self.b_name]
        i=sigmoid(activation[:,:self.h_dim])
        f=sigmoid(activation[:,self.h_dim:2*self.h_dim])
        o=sigmoid(activation[:,2*self.h_dim:3*self.h_dim])
        g=np.tanh(activation[:,3*self.h_dim:])
        next_c=np.multiply(f,prev_c)+np.multiply(i,g)
        next_h=np.multiply(o,np.tanh(next_c))
        meta=i,f,o,g,next_c,next_h,activation,prev_h,prev_c,x
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        pass
        i,f,o,g,next_c,next_h,activation,prev_h,prev_c,x=meta
        do=np.multiply(np.tanh(next_c),dnext_h)
        dnext_c+=np.multiply((np.multiply(o,(1-np.tanh(next_c)**2))),dnext_h)
        di=np.multiply(g,dnext_c)
        df=np.multiply(prev_c,dnext_c)
        dg=np.multiply(i,dnext_c)
        dprev_c=np.multiply(f,dnext_c)
        
        dg_hat=np.multiply((1-np.power(g,2)),dg)
        di_hat=np.multiply(np.multiply(i,(1-i)),di)
        df_hat=np.multiply(np.multiply(f,(1-f)),df)
        do_hat=np.multiply(np.multiply(o,(1-o)),do)
        dz=np.hstack((di_hat,df_hat,do_hat,dg_hat))
        
        dWx=np.dot(x.T,dz)
        dWh=np.dot(prev_h.T,dz)
        dprev_h=np.dot(dz,self.params[self.wh_name].T)
        dx=np.dot(dz,self.params[self.wx_name].T)
        db=np.sum(dz,axis=0)
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def forward(self, x, h0):
        ...
        N,H=h0.shape
        x=x.transpose(1,0,2)
        c0=np.zeros((x.shape[1],self.h_dim))
        prev_c=c0
        prev_h=h0
...
#         print('prev_h',prev_h.shape)
    ...
    def backward(self, dh):
        ...
        for i in range(dh.shape[0],0,-1):
            dxi, dprev_h, dprev_c, dWx, dWh, db=self.step_backward(dprev_h,dprev_c,self.meta[i-1])
            self.grads[self.wx_name]+=dWx
            self.grads[self.wh_name]+=dWh
            self.grads[self.b_name] +=db
            dh0=dprev_h
            dprev_h=dh[i-2]+dprev_h
            dprev_c=dprev_c
            dx[i-1]+=dxi
